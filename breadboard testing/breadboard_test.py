...
Peripherals tested:
  - ADS1115 ADC   (I2C 0x48): reads A0, back-calculates 24V battery voltage
  - SSD1306 OLED  (I2C 0x3C, 128x64): displays live ADC/battery state
  - GPIO24 button (internal pull-up, active LOW): prints to terminal + OLED

Wiring:
  ADS1115 A0  -> voltage divider output (divides 24V battery to 20% = ~4.8V max)
...
Install (Pi 5, Bookworm):
  ...
  sudo pip3 install rpi-lgpio adafruit-blinka \
      adafruit-circuitpython-ssd1306 \
      adafruit-circuitpython-ads1x15 \
      --break-system-packages
  sudo raspi-config nonint do_i2c 0
  # then reboot, verify with: i2cdetect -y 1  (expect 0x3c and 0x48)
"""
...
_btn_event      = threading.Event()   # set by button thread, cleared by main loop
...
def draw_oled(v_adc: float, v_batt: float, btn_label: str) -> None:
...
def button_thread() -> None:
    while not _shutdown.is_set():
        triggered = GPIO.wait_for_edge(BUTTON_PIN, GPIO.FALLING, timeout=500)
...
def main() -> None:
    ...
    print(f"  ADC     : ADS1115 A0 @ I2C 0x{ADS1115_ADDR:02X}, gain=2/3 (±6.144 V), divider={ADC_DIVIDER}")
    ...
    try:
        while True:
            ...
            draw_oled(v_adc, v_batt, btn_label)
